...
// The API key is sourced securely from the environment variables.
// Do not hardcode the API key in the code. It is automatically configured in the execution environment.
...
const parseResume = async (prompt: string, isFile: boolean = false) => {
    try {
...
${suggestions}`;
        ...
        const jsonStr = response.text.trim();
        const updatedResume = JSON.parse(jsonStr);
