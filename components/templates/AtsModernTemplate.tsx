...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-4">
        <h2 className="text-xs font-bold uppercase tracking-widest text-[color:var(--accent-color)] border-b border-gray-200 pb-1 mb-2">{title}</h2>
        {children}
    </section>
);
...
const AtsModernTemplate: React.FC<{ resume: Resume }> = React.memo(({ resume }) => {
        ...
        <div className="resume-template-body font-sans text-[9pt] text-gray-800 leading-normal" style={{ padding: `${resume.design.topBottomMargin}in ${resume.design.leftRightMargin}in`, fontFamily: resume.design.fontFamily, fontSize: resume.design.fontSize, lineHeight: resume.design.lineHeight }}>
            <header className="mb-6">
                <h1 className="text-2xl font-bold">{personalInfo.name}</h1>
                 {personalInfo.targetTitle && <p className="mt-1 text-md font-semibold text-gray-600">{personalInfo.targetTitle}</p>}
                ...
                {publications.map(pub => (
                    <div key={pub.id} className="mb-2 text-xs page-break-inside-avoid">
                        <div className="flex justify-between items-baseline">
                            ...
                            <p className="text-gray-600 font-medium">{pub.date}</p>
                        </div>
