...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-4">
        <h2 className="text-xs font-bold uppercase tracking-wider text-black border-b border-black pb-1 mb-2">{title}</h2>
        {children}
    </section>
);
...
const ModernProfessionalTemplate: React.FC<{ resume: Resume }> = React.memo(({ resume }) => {
    const { personalInfo, experience, education, skills, projects, certifications, volunteering, publications } = resume;

    return (
        <div className="resume-template-body font-serif text-[10pt] text-gray-800 leading-normal" style={{ padding: `${resume.design.topBottomMargin}in ${resume.design.leftRightMargin}in`, fontFamily: "'Times New Roman', serif" }}>
            <header className="text-center mb-6">
                <h1 className="text-3xl font-bold">{personalInfo.name}</h1>
                <div className="flex justify-center items-center gap-x-3 gap-y-1 text-xs flex-wrap mt-1 break-words">
                ...
                {projects.map(proj => (
                    <div key={proj.id} className="mb-3 text-xs page-break-inside-avoid">
                        <div className="flex items-baseline gap-2">
                            ...
                            {proj.url && <a href={formatUrl(proj.url)} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-[9pt] break-all">{displayUrl(proj.url)}</a>}
                        </div>
                        <ul className="list-disc list-inside mt-1 space-y-0.5">
