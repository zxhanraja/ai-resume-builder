...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-4">
        <h2 className="text-xs font-bold uppercase tracking-widest text-[color:var(--accent-color)] pb-1 mb-2">{title}</h2>
        {children}
    </section>
);
...
const QuantumTemplate: React.FC<{ resume: Resume }> = React.memo(({ resume }) => {
    ...
    return (
        <div className="resume-template-body font-sans text-[9pt] text-gray-800 leading-normal" style={{ padding: `${resume.design.topBottomMargin}in ${resume.design.leftRightMargin}in` }}>
            <header className="flex flex-col sm:flex-row justify-between items-start mb-6 border-b-2 pb-4">
            ...
            <Section title="Experience">
                {experience.map(exp => (
                    <div key={exp.id} className="mb-3 text-xs page-break-inside-avoid">
                        ...
                        <ul className="list-[square] list-inside mt-1 space-y-0.5">
